...
import { Instagram, ArrowLeft, Download, Loader2, Link2, Zap, HardDrive } from 'lucide-react'
...
export default function InstagramDownloaderPage() {
    ...
    const handleDownload = async () => {
        ...
        setResult(null)
        setProgress({ status: 'starting', message: 'Connecting to Instagram...' })

        if (eventSourceRef.current) {
            eventSourceRef.current.close()
        }

        eventSourceRef.current = videoApi.downloadInstagramWithProgress(
            url,
            ...
            <div className="max-w-4xl mx-auto px-6">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="tool-card-icon w-14 h-14 !bg-gradient-to-br from-pink-500/20 to-purple-500/10 !border-pink-500/30">
                            <Instagram className="w-7 h-7 text-pink-400" />
                        </div>
                        <div>
                            <h1 className="text-3xl font-bold">Instagram Downloader</h1>
                            <p className="text-[var(--text-muted)]">Download videos from Instagram posts</p>
                        </div>
                    </div>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="space-y-6">
                    <div className="glass-panel p-6">
                        <h2 className="text-lg font-semibold mb-4">Enter Instagram URL</h2>
                    ...
                    {!progress && !result && (
                        <Button
                            ...
                            disabled={!url.trim()}
                        >
                            {isProcessing ? <><Loader2 className="w-5 h-5 animate-spin" /> Downloading...</> : <><Instagram className="w-5 h-5" /> Download Video</>}
                        </Button>
                    )}
                    ...
                    {result && (
                        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel p-6">
                            ...
                            <Button className="w-full" onClick={() => downloadFile(result.filename, 'instagram_video.mp4')}>
